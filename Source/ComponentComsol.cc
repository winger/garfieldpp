...
ComponentComsol::ComponentComsol(std::string mesh, std::string mplist, std::string field) :
  ComponentFieldMap() {
...
bool
ComponentComsol::Initialise(std::string mesh, std::string mplist, std::string field) {
  double unit = 100.0; //m
  ...
  double minx = 1e100, miny = 1e100, minz = 1e100, maxx = -1e100, maxy = -1e100, maxz = -1e100;
  for (int i = 0; i < nNodes; ++i) {
    ...
    minx = std::min(minx, newNode.x); maxx = std::max(maxx, newNode.x);
    miny = std::min(miny, newNode.y); maxy = std::max(maxy, newNode.y);
    minz = std::min(minz, newNode.z); maxz = std::max(maxz, newNode.z);
  }
  ...
  std::getline(fmesh, line);
  // elements 6 & 7 are swapped due to differences in COMSOL and ANSYS representation
  ...
  for (int i = 0; i < nElements; ++i) {
    int domain;
    fmesh >> domain;
    elements[i].matmap = domain2material.count(domain) ? domain2material[domain] : m_nMaterials - 1;
  }
  fmesh.close();
  ...
  if (ffield.fail()) {
    std::cerr << m_className << "::Initialise:\n";
    std::cerr << "    Could not open field potentials file "
              << field << " for reading.\n";
    return false;
  }
  do {
    std::getline(ffield, line);
  } while (line.substr(0, 81) != "% x                       y                        z                        V (V)");
  {
  ...
  for (int i = 0; i < nNodes; ++i) {
    ...
    for (int j : nodeIdx[tmp]) {
      double dist = (tmp.x - nodes[j].x) * (tmp.x - nodes[j].x) + (tmp.y - nodes[j].y) * (tmp.y - nodes[j].y) + (tmp.z - nodes[j].z) * (tmp.z - nodes[j].z);
      if (dist < closestDist) {
        closestDist = dist;
        closest = j;
      }
...
bool
ComponentComsol::SetWeightingField(std::string field, std::string label) {
  double unit = 100.0; // m;
...
void
ComponentComsol::ElectricField(const double x, const double y, const double z,
...
ComponentComsol::ElectricField(
  ...
  if (imap < 0) {
//    std::cout << "ef @(" << xin << ", " << yin << ", " << zin << ") = not in mesh" << "\n";
    if (m_debug) {
...
void
ComponentComsol::WeightingField(
                    const double xin, const double yin, const double zin,
...
double
ComponentComsol::WeightingPotential(
                    const double xin, const double yin, const double zin,
                    const std::string& label) {
...
ComponentComsol::GetMedium(
  ...
  if (elements[imap].matmap < 0 || elements[imap].matmap >= m_nMaterials ) {
    if (m_debug) {
      std::cerr << m_className << "::GetMedium:\n";
      std::cerr << "    Point (" << x << ", " << y 
...
double
ComponentComsol::GetElementVolume(const int i) {
...
void
ComponentComsol::GetAspectRatio(const int i, double& dmin, double& dmax) {
